...
# carries `requires_orchid=>=1.0` and passes (ORCHID_VERSION=1.0.0-m2
...
assert_eq 13 "$?" "requires_orchid '>=2.0' unmet by orchid 1.0.0-m2 rejects with exit 13"
...
# p17 above only exercised a major-version bump (>=2.0 vs 1.0.0-m2), which
...
# ORCHID_VERSION 1.0.0-m2 (minor 0) must still reject (0 < 10 numerically).
...
assert_eq 13 "$?" "requires_orchid '>=1.10' unmet by orchid 1.0.0-m2 rejects with exit 13"
...
# ORCHID_VERSION is fixed at 1.0.0-m2 for the rest of this suite, so its
