...
mkdir -p "$rt/packs/j-e1-TPEND-a1-pend0001"
echo '{}' > "$rt/requests/j-e1-TPEND-a1-pend0001.json"
echo pend-log > "$rt/logs/j-pend.log"
jq -n --argjson pid "$pend_pid" --argjson started "$old_started" --arg log "$rt/logs/j-pend.log" \
  --arg out "$rt/spool/j-e1-TPEND-a1-pend0001.json" \
  '{job_id:"j-e1-TPEND-a1-pend0001", task:"TPEND", attempt:1, role:"implementer", operation:"implement",
    engine:"fake", pid:$pid, pgid:0, started_at:$started, log:$log, output:$out,
    base_sha:"", candidate_sha:""}' \
  > "$rt/jobs/j-pend.json"
echo '{"contract":1,"job_id":"j-e1-TPEND-a1-pend0001","task":"TPEND","operation":"implement","status":"ok"}' \
  > "$rt/spool/j-e1-TPEND-a1-pend0001.json"

pend_out="$("$ORCHID_BIN" jobs gc --older-than-s 86400)"
assert_match "gc-pending j-e1-TPEND-a1-pend0001" "$pend_out" \
  "gc names the manifest it is holding back, and why"
echo "$pend_out" | grep -q "^gc j-e1-TPEND-a1-pend0001$" \
...
[ -f "$rt/spool/j-e1-TPEND-a1-pend0001.json" ] || fail "gc: the pending envelope itself must be untouched"
[ -d "$rt/packs/j-e1-TPEND-a1-pend0001" ] || fail "gc: nor may the pack the envelope's job was built from be reaped"
[ -f "$rt/requests/j-e1-TPEND-a1-pend0001.json" ] || fail "gc: nor its request document"

# Drained (what reconcile does with it): the hold is released and the ordinary
# dead-job reap applies, unchanged.
rm -f "$rt/spool/j-e1-TPEND-a1-pend0001.json"
pend_out2="$("$ORCHID_BIN" jobs gc --older-than-s 86400)"
assert_match "^gc j-e1-TPEND-a1-pend0001$" "$pend_out2" \
