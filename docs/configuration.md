...
- **`cpu_stall_min_s`** (default `1`) is the CPU floor of the stall check.
  `stall_minutes` catches a job that stops writing to its log; this catches
  the job that keeps writing and stops working — an engine still emitting
  heartbeats while its cumulative CPU time barely moves. `orchid jobs check`
  reads the `cpu` field of the heartbeat lines already in every job log and
  reports `stalled` (and kills the job, exactly as the log-mtime arm does)
  when the job burned less than this many CPU-seconds across the last
  `stall_minutes` of heartbeats. It is deliberately far below what any
  working engine accumulates, because an engine blocked on a vendor API
  legitimately burns very little CPU; raise it only if you know your engines'
  profile, and set it to `0` to disable the check outright.
