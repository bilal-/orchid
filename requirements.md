...
- Add a deterministic, per-repository acknowledgement gate before unattended
  headless ticks or service installation may run. Interactive/manual operation
  must remain available without silently opting into unattended trust.
...
- A fresh repo cannot run `runners/orchid-tick` or install the background
  service until the operator explicitly acknowledges unattended trust in
