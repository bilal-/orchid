...
_unattended_root_commit() {
  ...
  git -C "$repo" rev-list --max-parents=0 HEAD 2>/dev/null \
    | LC_ALL=C sort \
    | awk 'NF { if (roots != "") roots=roots ","; roots=roots $0 }
           END { if (roots != "") print roots }'
}

_unattended_record_path_for_identity() {
  printf '%s/%s-%s.json\n' "$(_unattended_trust_dir)" "$1" "$2"
}
...
unattended_trust_inspect() {
  local repo_in="$1" ident rec_schema rec_device rec_inode rec_policy rec_root
  ...
  ORCHID_UNATTENDED_RECORD="$(_unattended_record_path_for_identity \
  ...
  elif [ -z "$ORCHID_UNATTENDED_ACKNOWLEDGED_AT" ] || [ -z "$ORCHID_UNATTENDED_REASON" ]; then
    ORCHID_UNATTENDED_STATE=invalid
...
unattended_trust_summary_loaded() {
  ...
  if [ "$ORCHID_UNATTENDED_STATE" = trusted ]; then
    ...
    printf 'denied — %s; git-common identity %s:%s; root %s; policy %s; acknowledge with: orchid trust unattended <repo> --reason <reason>\n' \
...
unattended_trust_show() {
  ...
  if [ -n "$ORCHID_UNATTENDED_ACKNOWLEDGED_AT" ]; then
...
unattended_trust_acknowledge() {
  ...
  [ -n "$reason" ] || orchid_die "unattended trust requires a non-empty --reason"
